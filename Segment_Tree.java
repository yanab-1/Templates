class Segment_Tree {
    ...
    public void update_Point(int i, int val){
        update_Point(i, val, 0, n - 1, 0);
    }

    private void update_Point(int i, int val, int l, int r, int idx){
        // lazy update
        lazy_Update(l, r, idx);
        if(l == r){
            tree[idx] = val;
        ...
        int mid = (l + r) / 2;
        if(i <= mid){
            update_Point(i, val, l, mid, 2 * idx + 1);
        }
        else{
            update_Point(i, val, mid + 1, r, 2 * idx + 2);
        }
        tree[idx] = tree[2 * idx + 1] + tree[2 * idx +  2]; // change according to question
    }
